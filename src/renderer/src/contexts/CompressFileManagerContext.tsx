...
/**
 * CompressFileManager Context 接口
 */
interface CompressFileManagerContextType {
...
const CompressFileManagerContext = createContext<CompressFileManagerContextType | undefined>(
  undefined
);

/**
 * CompressFileManagerProvider
 * 为 Compress 页面提供独立的文件列表管理
 */
export function CompressFileManagerProvider({ children }: { children: ReactNode }) {
  ...
  const handleFilesSelected = async (files: File[]) => {
    ...
    for (const file of files) {
      ...
      if ('path' in file && file.path) {
        ...
        try {
          ...
          logger.errorFromCatch('CompressFileManager', `无法获取文件路径: ${file.name}`, error);
          continue;
        }
      }
      ...
      if (existingPaths.has(filePath)) {
        duplicateCount++;
        logger.info('CompressFileManager', `跳过重复文件: ${file.name} (${filePath})`);
        continue;
      }
    ...
    // 记录去重信息
    if (duplicateCount > 0) {
      logger.info('CompressFileManager', `过滤了 ${duplicateCount} 个重复文件`);
      toast.info('跳过重复文件', {
    ...
    for (let i = 0; i < newFiles.length; i++) {
      ...
      try {
        logger.info('CompressFileManager', `获取文件媒体信息: ${filePath}`);
  ...
  const handleFilesFromDialog = async (fileInfos: FileInfo[]) => {
    ...
    for (const fileInfo of fileInfos) {
      ...
      if (existingPaths.has(filePath)) {
        duplicateCount++;
        logger.info('CompressFileManager', `跳过重复文件: ${fileInfo.name} (${filePath})`);
        continue;
      }
  ...
  const value: CompressFileManagerContextType = {
    selectedFiles,
    selectedFile,
    setSelectedFile,
    handleFilesSelected,
  ...
  return (
    <CompressFileManagerContext.Provider value={value}>
      {children}
    </CompressFileManagerContext.Provider>
  );
}

/**
 * useCompressFileManager Hook
 * 在 Compress 页面中使用此 Hook 获取文件管理功能
 */
export function useCompressFileManager() {
  const context = useContext(CompressFileManagerContext);
  if (context === undefined) {
    throw new Error('useCompressFileManager must be used within CompressFileManagerProvider');
  }
  return context;
}
